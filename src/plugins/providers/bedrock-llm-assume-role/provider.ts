/**
 * Bedrock LLM Provider with AssumeRole Support
 *
 * This provider extends the standard Bedrock provider with support for:
 * - AWS AssumeRole for cross-account access
 * - SSO credential integration
 * - Custom inference ARNs
 */
...
export class BedrockLLMProvider {
  ...
  constructor(options: BedrockLLMOptions, debugStore?: any) {
    const {
      modelName,
      region = 'us-east-1',
      sourceProfile,
    ...
    // Build credential provider chain
    let credentials;

    if (roleArn && sourceProfile) {
      // Assume role configuration
      console.log(`[BedrockLLMProvider] Using AssumeRole: ${roleArn} from profile ${sourceProfile}`);
      ...
      // Standard profile (supports SSO via ~/.aws/config)
      console.log(`[BedrockLLMProvider] Using profile: ${sourceProfile}`);

      credentials = fromIni({ profile: sourceProfile });
    } else {
      // Default credential chain (env vars, ~/.aws/credentials, IAM roles)
      console.log(`[BedrockLLMProvider] Using default credential chain`);
      credentials = fromIni();
    }
  ...
  get name() {
    return 'bedrock-llm-assume-role';
  }
  ...
  async summarize(text: string, maxLength?: number): Promise<string> {
    ...
    try {
      ...
      console.error('[BedrockLLMProvider] Error in summarize:', error);
      throw error;
    }
  }
  ...
  async generateBestQuestion(code: string, summary: string): Promise<string> {
    ...
    try {
      ...
      console.error('[BedrockLLMProvider] Error in generateBestQuestion:', error);
      throw error;
    }
  }
  ...
  async generateResponse(prompt: string, context?: string): Promise<string> {
    ...
    try {
      ...
      console.error('[BedrockLLMProvider] Error in generateResponse:', error);
      throw error;
    }
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      console.log('[BedrockLLMProvider] Connection test successful');
      return true;
    } catch (error) {
      console.error('[BedrockLLMProvider] Connection test failed:', error);
      return false;
    }
  }
  ...
  private async callBedrock(prompt: string): Promise<string> {
    ...
    console.warn('[BedrockLLMProvider] Unexpected response format:', responseBody);
