/**
 * Bedrock LLM Provider Plugin
 *
 * Provider plugin that enables AWS Bedrock LLM with AssumeRole and SSO support.
 */
...
const plugin: ProviderPlugin = {
  ...
  configSchema: {
    fields: [
      ...
      },
      {
        name: 'sourceProfile',
        type: 'text',
        label: 'Source SSO Profile',
        placeholder: 'my-sso-profile',
        required: false,
        helperText: 'AWS profile name from ~/.aws/config (with SSO or credentials)'
      },
      {
        name: 'roleArn',
        type: 'arn',
        label: 'Role ARN to Assume',
        placeholder: 'arn:aws:iam::999999999999:role/BedrockAccessRole',
        required: false,
        helperText: 'IAM role ARN to assume in the target account'
      },
      {
        name: 'ssoSession',
        type: 'text',
  ...
  async validateCredentials(config: any): Promise<boolean> {
    try {
      ...
      console.error('[BedrockLLMPlugin] Credential validation failed:', error);
      return false;
    }
  },
