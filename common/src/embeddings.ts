...
import { EmbeddingProvider, SummarizerProvider, ProviderConfig, ChatMessage } from "./providers/base.js";
  ...
  async setProvider(config: ProviderConfig, throttlingErrors: string[] = []) {
    ...
    if (config.pluginName) {
      ...
      if (plugin) {
        ...
        if ((this.provider as any).initialize) {
          await (this.provider as any).initialize(config);
        }
