...
import { VectorDBProvider, DBConfig, VectorResult, SearchOptions } from "./database/base.js";
...
export async function hybridSearch(queryText: string, embedding: number[], options: SearchOptions = {}) {
...
export async function search(embedding: number[], options: SearchOptions = {}) {
...
export async function compactDatabase() {
  ...
  if (p instanceof LanceDBProvider) {
    ...
    if (table) {
      ...
      try {
        await (table as any).cleanupOldVersions();
        await (table as any).compactFiles();
      } catch { }
    }
  }
