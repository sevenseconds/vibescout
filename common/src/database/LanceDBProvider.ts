...
export class LanceDBProvider implements VectorDBProvider {
  ...
  async insert(data: VectorResult[]): Promise<void> {
    ...
    if (tables.includes(tableName)) {
      ...
      try {
        await table.add(data as any as Record<string, unknown>[]);
      } catch (err: any) {
        ...
        if (err.message.includes("Found field not in schema: category")) {
          ...
          const newTable = await db.createTable(tableName, [...migratedData, ...data] as any as Record<string, unknown>[]);
      ...
      const table = await db.createTable(tableName, data as any as Record<string, unknown>[]);
